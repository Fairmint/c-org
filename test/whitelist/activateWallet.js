...
contract("whitelist / activateWallet", (accounts) => {
  ...
  it("shouldFail when msg.sender is not callingContract nor owner", async () => {
    await expectRevert(contracts.whitelist.activateWallet(accounts[4],{from:accounts[4]}),"CALL_VIA_CONTRACT_OR_OPERATOR_ONLY");
  });
  
  it("shouldFail when wallet does not have approvedUserId", async () => {
    await expectRevert(contracts.whitelist.activateWallet(accounts[9],{from:operatorAccount}),"USER_UNKNOWN");
  });
  
  it("shouldFail when wallet is already activated", async () => {
    await contracts.whitelist.activateWallet(accounts[4], {from:operatorAccount});
    await expectRevert(contracts.whitelist.activateWallet(accounts[4],{from:operatorAccount}),"ALREADY_ACTIVATED_WALLET");
  });
  ...
  it("should automatically activate from, to wallets when transfer triggered", async () => {
    ...
    assert.equal(await contracts.whitelist.authorizedWalletToUserId(accounts[4]), accounts[5]);
    assert.equal(await contracts.whitelist.authorizedWalletToUserId(accounts[6]), accounts[5]);
    const price = web3.utils.toWei("100", "ether");
    const receipt = await contracts.dat.buy(accounts[6], price, 1, {
      from: accounts[6],
      value: price,
    });
    assert.equal(await contracts.whitelist.investorEnlisted(accounts[5]), true);
    assert.equal(await contracts.whitelist.userActiveWalletCount(accounts[5]), 1);
  ...
  it("should increase userActiveWalletCount", async () => {
    ...
    assert.equal(await contracts.whitelist.walletActivated(accounts[4]), true);
    assert.equal(await contracts.whitelist.userActiveWalletCount(accounts[5]), 2);
  });

  it("should enlist user if user is delisted when wallet == user", async () => {
    assert.equal(await contracts.whitelist.investorEnlisted(accounts[7]), false);
    assert.equal(await contracts.whitelist.walletActivated(accounts[7]), false);
    assert.equal(await contracts.whitelist.authorizedWalletToUserId(accounts[7]), accounts[7]);
    const price = web3.utils.toWei("100", "ether");
    const receipt = await contracts.dat.buy(accounts[7], price, 1, {
      from: accounts[7],
      value: price,
    });
    assert.equal(await contracts.whitelist.investorEnlisted(accounts[7]), true);
    assert.equal(await contracts.whitelist.userActiveWalletCount(accounts[7]), 1);
  ...
  it("should enlist user if user is delisted when wallet != user", async () => {
    assert.equal(await contracts.whitelist.investorEnlisted(accounts[7]), false);
    assert.equal(await contracts.whitelist.walletActivated(accounts[7]), false);
    assert.equal(await contracts.whitelist.authorizedWalletToUserId(accounts[8]), accounts[7]);
    const price = web3.utils.toWei("100", "ether");
    const receipt = await contracts.dat.buy(accounts[8], price, 1, {
      from: accounts[8],
      value: price,
    });
    assert.equal(await contracts.whitelist.investorEnlisted(accounts[7]), true);
  ...
  it("should fail if currentUser exceeds total limit", async ()=>{
    ...
    await contracts.whitelist.setInvestorLimit(curInvestors, {from:ownerAccount});
    const price = web3.utils.toWei("100", "ether");
    await expectRevert(contracts.dat.buy(accounts[7], price, 1, {
      from: accounts[7],
      value: price,
  ...
  it("should fail if currentUser of jurisdiction exceeds jurisdiction limit", async () => {
    const curInvestors = await contracts.whitelist.currentInvestorsByJurisdiction(4);
    await contracts.whitelist.setInvestorLimitForJurisdiction(4, curInvestors, {from:ownerAccount});
    const price = web3.utils.toWei("100", "ether");
    await expectRevert(contracts.dat.buy(accounts[7], price, 1, {
      from: accounts[7],
      value: price,
