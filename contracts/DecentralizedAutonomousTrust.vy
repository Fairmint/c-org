...
def _collectInvestment(
  ...
  if(self.currency == ZERO_ADDRESS): # currency is ETH
    if(_refundRemainder):
      ...
      refund: uint256 = _msgValue - _quantityToInvest
      if(refund > 0):
        raw_call(_from, b"", outsize=4096, value=refund, gas=30000)
    else:
...
def _sendCurrency(
  ...
  if(_amount > 0):
    if(self.currency == ZERO_ADDRESS):
      raw_call(_to, b"", outsize=4096, value=as_wei_value(_amount, "wei"), gas=30000)
    else:
      if(self.isCurrencyERC777):
