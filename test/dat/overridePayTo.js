...
contract("dat / overridePayTo", (accounts) => {
  ...
  const overridePayTo = accounts[8];
  ...
  it("The overridePayTo address was set", async () => {
    const payTo = await contracts.dat.overridePayTo();
    assert.equal(payTo, overridePayTo);
  });

  describe("on pay", () => {
    let investorBalanceBefore;
    let payValue;
    let payToBalanceBefore;

    beforeEach(async () => {
      investorBalanceBefore = new BigNumber(
        await contracts.dat.balanceOf(investor)
      );
      payToBalanceBefore = new BigNumber(
        await contracts.dat.balanceOf(overridePayTo)
      );
      payValue = new BigNumber(await contracts.dat.estimatePayValue(payAmount));
    });

    describe("with pay to investor specified", () => {
      beforeEach(async () => {
      ...
      it("The payTo balance went up", async () => {
        const balance = new BigNumber(
          await contracts.dat.balanceOf(overridePayTo)
        );
        assert.equal(
          balance.toFixed(),
          payToBalanceBefore.plus(payValue).toFixed()
        );
      });
      ...
      it("shouldFail if currencyValue is missing", async () => {
        ...
        await token.mint(accounts[0], constants.MAX_UINT, {
          from: accounts[0],
        });
    ...
    describe("with pay to not specified", () => {
      beforeEach(async () => {
        await contracts.dat.pay(constants.ZERO_ADDRESS, payAmount, {
          from: investor,
          value: payAmount,
        });
      });
