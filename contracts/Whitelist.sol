...
contract Whitelist is IWhitelist, Ownable, OperatorRole {
  ...
  event ApproveNewUser(
    ...
    uint256 indexed _jurisdictionId,
    address indexed _operator
  );
   ...
   */
  mapping(uint256 => mapping(uint256 => uint256)) internal jurisdictionFlows;
  ...
  struct Lockup {
    ...
    uint256 lockupExpirationDate;
    // How many tokens locked until the given expiration date.
    uint256 numberOfTokensLocked;
  }
   ...
   */
  mapping(address => mapping(uint256 => Lockup)) internal userIdLockups;
  ...
  function getJurisdictionFlow(
    uint256 _fromJurisdictionId,
    uint256 _toJurisdictionId
  ) external view returns (uint256 lockupLength) {
   ...
   */
  function getUserIdLockup(address _userId, uint256 _lockupIndex)
    external
    view
  ...
  function detectTransferRestriction(
    ...
    if (fromUserId != toUserId) {
      uint256 fromJurisdictionId = authorizedUserIdInfo[fromUserId]
        .jurisdictionId;
      uint256 toJurisdictionId = authorizedUserIdInfo[toUserId].jurisdictionId;
   ...
   */
  function configWhitelist(uint256 _startDate, uint256 _lockupGranularity)
    external
    onlyOwner()
  {
   ...
   */
  function processLockups(address _userId, uint256 _maxCount) external {
    UserInfo storage info = authorizedUserIdInfo[_userId];
    require(info.jurisdictionId > 0, "USER_ID_UNKNOWN");
   ...
   */
  function forceUnlockUpTo(address _userId, uint256 _maxLockupIndex)
    external
    onlyOperator()
  {
    UserInfo storage info = authorizedUserIdInfo[_userId];
  ...
  function authorizeTransfer(
    ...
    if (fromUserId != toUserId) {
      ...
      if (_from == address(0)) {
        ...
        uint256 balance = callingContract.balanceOf(_from);
