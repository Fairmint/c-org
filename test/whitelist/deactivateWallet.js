...
contract("whitelist / deactivateWallet", (accounts) => {
  ...
  it("shouldFail if wallet has balance", async ()=>{
    await expectRevert(contracts.whitelist.deactivateWallet(accounts[4],{from:operatorAccount}),"ATTEMPT_TO_DEACTIVATE_WALLET_WITH_BALANCE");
  });
  
  it("shouldFail if wallet does not have approvedUserId", async ()=>{
    await expectRevert(contracts.whitelist.deactivateWallet(accounts[9],{from:operatorAccount}),"USER_UNKNOWN");
  });
  
  it("shouldFail when wallet is already deactivated", async () => {
    await expectRevert(contracts.whitelist.deactivateWallet(accounts[8],{from:operatorAccount}),"ALREADY_DEACTIVATED_WALLET");
  });
  ...
  it("should delist user if user has no more active wallet", async () => {
    ...
    assert.equal(await contracts.whitelist.userActiveWalletCount(accounts[5]), 0);
    assert.equal(await contracts.whitelist.investorEnlisted(accounts[5]), false);
  });
});
