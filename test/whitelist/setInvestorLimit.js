...
contract("whitelist / setInvestorLimit", (accounts) => {
  let contracts;
  let ownerAccount;
  ...
  it("shouldFail when msg.sender is not owner", async () => {
    await expectRevert(contracts.whitelist.setInvestorLimit(10,{from:accounts[4]}),"Ownable: caller is not the owner");
  });

  it("shouldFail if limit is less than currentInvestors", async () => {
    await expectRevert(contracts.whitelist.setInvestorLimit(1,{from:ownerAccount}),"LIMIT_SHOULD_BE_LARGER_THAN_CURRENT_INVESTORS");
  });

  it("should update maxInvestors", async () => {
    await contracts.whitelist.setInvestorLimit(10,{from:ownerAccount});
    assert.equal(await contracts.whitelist.maxInvestors(), 10);
  });
});
